<?php
header("Access-Control-Allow-Origin: https://regcard.dafam.cloud");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    addSignatureToPdf($inputPdfFilename, $signatureFilePath, $outputPdfFilePath, $email, $no_telp);
    ...
    try {
        ...
        $stmt = $connection->prepare("UPDATE regform SET signature_path = ?, no_telp = ?, at_regform = ?, email = ? WHERE id = ?");
        if ($stmt === false) {
            throw new Exception("Prepare failed: " . $connection->error);
        }

        $stmt->bind_param("sssss", $signatureFilename, $no_telp, $at_regform, $email, $device_id);
        ...
        $truncateStmt = $connection->prepare("TRUNCATE TABLE device");
...
function addSignatureToPdf($inputPdfPath, $signatureImagePath, $outputPdfPath, $email, $no_telp) {
    $pdf = new Fpdi();
    ...
    for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
        ...
        if ($pageNo == $pageCount) {
            ...
            $pdf->Text(137, 56, 'EMAIL      ' . $email);
            $pdf->Text(137, 61, 'MOBILE     ' . $no_telp);
        }
    }
